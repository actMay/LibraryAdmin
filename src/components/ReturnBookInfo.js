...
export default class UserInfo extends Component{
  ...
  EdithandleOk() {
    ...
    if(EditBorId&&EditBorBookId&&EditBorTime&&EditBorPerId&&EditBorDetail) {
        axios.post('/api/returnBook/EditBor',{
  ...
  handleOk() {
    ...
    if(userId&&userName&&userPass&&userType&&userSex) {
        axios.post('/api/returnBook/addBor',{
          'userId': userId,
          'userName': userName,
          'userPass': userPass,
          'userType': userType,
  ...
  userSearch() {
    var that = this;
    axios.get('/api/returnBook/searchBor',{
      params: {
  ...
  UserEdit(value) {
    ...
    console.log(value)
    axios.get('/api/returnBook/EditCurrentBorInfo',{
      params: {
        'borId': value
      }
    }).then((response) =>{
  ...
  confirm(value) {
    ...
    axios.post('/api/returnBook/delBor', {
      'userId': value
    }).then((response) => {
      if(response.data.resultCode=='000000') {
        this.getData()
      ...
      <div className="Breadcrumb">借阅信息/还书信息管理</div>
        <div>
          <div className="bookAdminSearch">
            ...
            <div className="bookInput">
              <Input placeholder="请输入书籍ID" type="text" ref="searchUserId"/>
            </div>
          </div>
          <div className="bookAdminSearch">
        ...
        >
          <div className="bookAdminForm">
            <div className="bookUserId">还书记录ID:</div>
            <div className="bookInput">
              <Input ref="BorId"/>
            </div>
          </div>
          ...
          <div className="bookAdminForm">
            <div className="bookUserId">还书时间:</div>
            <div className="bookInput">
              <Input ref="BorTime"/>
            </div>
          </div>
          <div className="bookAdminForm">
            <div className="bookUserId">还书人ID:</div>
            <div className="bookInput">
              <Input ref="BorPerId"/>
            </div>
          </div>
